...
const { data: country } = useAsyncData(async () => {
  return await fetchCountryByName(countryRepo, {
    countryName: Array.isArray(route.params.id)
      ? route.params.id[0]
      : route.params.id,
  });
});
...
<template>
  <nuxt-link to="/">
    <base-button back-btn>Back</base-button>
  </nuxt-link>
